...
from Legendbot.core.decorators import check_owner
from Legendbot.helpers import humanbytes, progress
from Legendbot.helpers.utils import _legendutils
...
# BACK
@legend.tgbot.on(CallbackQuery(pattern="fmback"))
@check_owner
...
# UP
@legend.tgbot.on(CallbackQuery(pattern="fmup_(.*)"))
@check_owner
async def up(event):
...
# DOWN
@legend.tgbot.on(CallbackQuery(pattern="fmdown_(.*)"))
@check_owner
...
# FORTH
@legend.tgbot.on(CallbackQuery(pattern="fmforth_(.*)"))
@check_owner
...
# REMOVE
@legend.tgbot.on(CallbackQuery(pattern="fmrem_(.*)"))
@check_owner
async def remove(event):
    ...
    await asyncio.sleep(1)
    await event.edit(msg, buttons=buttons)
    await _legendutils.runcmd(f"rm -rf '{rpath}'")
    await event.answer(f"{rpath} removed successfully...")


# SEND
@legend.tgbot.on(CallbackQuery(pattern="fmsend"))
@check_owner
...
# CUT
@legend.tgbot.on(CallbackQuery(pattern="fmcut_(.*)"))
@check_owner
async def cut(event):
...
# COPY
@legend.tgbot.on(CallbackQuery(pattern="fmcopy_(.*)"))
@check_owner
...
# PASTE
@legend.tgbot.on(CallbackQuery(pattern="fmpaste_(.*)"))
@check_owner
