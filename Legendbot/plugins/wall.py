...
async def noods(event):
    ...
    if not walls:
        return await eod(legendevent, f"**Can't find anything with** `{query}`", 10)
    i = count = 0
    piclist = []
    piclinks = []
    captionlist = []
    ...
    for x in walls:
        ...
        if "We are sorry," not in requests.get(a).text and a not in piclinks:
            await eor(legendevent, "📥** Downloading...**")
            pic = await wall_download(a, query)
            if pic is None:
                return await eod(legendevent, "__Sorry i can't download wallpaper.__")
            piclist.append(pic)
            piclinks.append(a)
            captionlist.append("")
            count += 1
            ...
            i += 1
        await eor(
            legendevent, f"**📥 Downloaded : {count}/{limit}\n\n❌ Errors : {i}/5**"
        )
        if count == int(limit):
            break
        if i == 5:
            await eor(legendevent, "`Max search error limit exceed..`")
    try:
        await eor(legendevent, "`Sending...`")
